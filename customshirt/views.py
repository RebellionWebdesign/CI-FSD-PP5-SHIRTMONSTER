    ...
    def post(self, request):
        ...
        if 'image' in request.FILES:
            try:
                ...
                image.seek(0)
                client.upload_fileobj(image, os.environ.get(
                    'AWS_BUCKET'), image_location)
                ...
                messages.success(request,
                                 'Inquiry saved, we will contact you'
                                 'within 24 hours!')
                return redirect(reverse('custom_shirts'))
            except Exception as e:
                messages.error(
                    request, f'Error while uploading image. See {e}')
        else:
            inquiry_form.save()
            messages.info(
                request, 'No image provided, submitting without image.')
